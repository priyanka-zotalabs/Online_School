...
export let enhancer = compose(
  ...
  (SignUpViaMobileNumberComponent) => ({ setAuthUser, ...props }) => {
    ...
    const verifyOtp = () => {
      ...
      setLoader(true)
      axios.post(`${appConfig.host}${urls.registerMobileOtp}`, params)
      .then(success => {
        
        setLoader(false)
        props.history.push("/student/signup/demography");
      }).catch(error => {
        setLoader(false)
        setError(error.response && error.response.data ? error.response.data.message : 'Invalid OTP')
      })
    }
