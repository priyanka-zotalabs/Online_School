...
export let enhancer = compose(
  ...
  (RegisterComponent) => ({ setAuthUser, ...props }) => {
    ...
    //   setLoader(true);
    //   axios
    //     .post(`${appConfig.host}${urls.registerUsingMobile}`, params)
    //     .then((success) => {
    //       setLoader(false);
    //       props.history.push("/student/signup/otp/" + params.contactNumber);
    //     })
    //     .catch((error) => {
    //       setLoader(false);
    ...
    const registerUsingEmailAddress = () => {
      ...
      setLoader(true);
      axios
        .post(`${appConfig.host}${urls.registerUsingEmail}`, params)
        .then((success) => {
          setLoader(false);
          props.history.push("/student/signup/EmailOtp/" + params.email);
        })
        .catch((error) => {
          setLoader(false);
          setError(error.response.data.message);
